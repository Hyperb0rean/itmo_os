...
fn panic(_: &::core::panic::PanicInfo) -> ! {
    write(std_fd::STDERR, b"Rust panic happened lol, you are shit");
    exit(1);
    loop {}
}
...
pub fn write(fd: FileDescriptor, input: &[u8]) {
    unsafe {
        raw_syscalls::write(fd.fd, input.as_ptr() as *const c_void, input.len() as i32);
    }
    
}

pub fn read(fd: FileDescriptor, output: &mut [u8], count: i32) {
    unsafe {
        raw_syscalls::read(fd.fd, output.as_ptr() as *mut c_void, count);
    }
}
...
pub fn pipe(p : &mut [i32]) {
    unsafe{
        raw_syscalls::pipe(p.as_ptr() as *mut i32);
    }
