...
fork(void)
{
  ...
  vmprint(p->pagetable);
  if(uvmcopy(p->pagetable, np->pagetable, p->sz, PTE_W,0) < 0){
    freeproc(np);
    release(&list_lock);
    return -1;
  }
...
dump2(int pid, int register_num, uint64* return_value)
{
  ...
  struct proc *p;
    ...
    for (l = list.next; l != &list; l = l->next) {
        ...
        if (p->pid == pid) {
            acquire(&list_lock);
            if (p != current_p && p->parent != current_p) {
                release(&list_lock);
                release(&p->lock);
                return FORBIDDEN;
            }
            uint64 * reg_ptr = &(p->trapframe->s2) + (register_num - 2);
            if (copyout(current_p->pagetable, (uint64) return_value, (char *) reg_ptr, sizeof(uint64)) != 0) {
                release(&list_lock);
                release(&p->lock);
