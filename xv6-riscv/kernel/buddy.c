...
int buddy_bit_isset(char *array, int index) {
  char b = array[index / 16];
  char m = (1 << ((index % 16) / 2));
  return (b & m) == m;
}
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_inverse(bd_sizes[k].xor, blk_index(k, p));
  ...
  for (; k > fk; k--) {
    ...
    bit_inverse(bd_sizes[k - 1].xor, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_inverse(bd_sizes[k].xor, bi);           // free p at size k
    if (buddy_bit_isset(bd_sizes[k].xor, buddy)) {  // is buddy allocated?
...
int bd_initfree_pair_right(int k, int bi) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (buddy_bit_isset(bd_sizes[k].xor, bi)) {
